    ...
    StartNewTree(const KeyType &key, const ValueType &value)
{
  ...
  if (page == nullptr)
  {
    throw Exception(EXCEPTION_TYPE_INDEX,
                    "out of memory while StartNewTree");
  }
  auto root =
    ...
    InsertIntoLeaf(const KeyType &key, const ValueType &value, Transaction *transaction)
{
  ...
  {// return false if the tree has key
...
{
  // if old_value represents the root, need to create one new root
